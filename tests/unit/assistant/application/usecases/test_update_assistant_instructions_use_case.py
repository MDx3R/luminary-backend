...
@pytest.mark.asyncio
class TestUpdateAssistantInstructionsUseCase:
    ...
    def setup(self) -> None:
        ...
        self.command = UpdateAssistantInstructionsCommand(
            user_id=self.user_id,
            assistant_id=self.assistant_id,
            prompt="New prompt",
        )

        self.use_case = UpdateAssistantInstructionsUseCase(
    ...
    async def test_updates_assistant_instructions(self) -> None:
        await self.use_case.execute(self.command)

        assert self.assistant.instructions.prompt == "New prompt"
